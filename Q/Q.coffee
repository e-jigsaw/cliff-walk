class Q
	...
	update: (reward, currentState, nextState, action)->
		maxAction = @decide nextState.x, nextState.y, false # calc max action
		@data[currentState.y][currentState.x][action].val += @alpha * (reward + @gamma * @data[nextState.y][nextState.x][maxAction].val - @data[currentState.y][currentState.x][action].val) # Q-learning update formura
